...
// Remove every escape sequence and every C0/C1/DEL control byte. The final
// control-byte strip is the hard guarantee — even an unterminated or malformed
// escape leaves no ESC/control behind.
function scrubControls(s: string): string {
  return s
    // eslint-disable-next-line no-control-regex
    .replace(/\x1b[\]P^_X][\s\S]*?(?:\x07|\x1b\\|$)/g, '') // OSC/DCS/PM/APC/SOS … ST/BEL/EOL
    // eslint-disable-next-line no-control-regex
    .replace(/\x1b\[[0-9;?]*[ -/]*[@-~]/g, '') // CSI (cursor move, erase, etc.)
    // eslint-disable-next-line no-control-regex
    .replace(/[\x00-\x1f\x7f\x80-\x9f]/g, '') // ALL remaining C0 (incl ESC, BEL, CR, LF, TAB), C1, DEL
}
...
/**
 * Make an externally-supplied string SAFE to render to a terminal. Item text in
 * pickem is frequently attacker-controllable (branch names, PR titles, filenames,
 * API results); rendered verbatim it is a terminal-control-injection primitive
 * (OSC clipboard writes, cursor-movement/erase UI spoofing, BEL flooding — CWE-150).
 *
 * Strips every ANSI escape sequence and every C0/C1/DEL control byte, EXCEPT SGR
 * (color/style) sequences, which are inert and let pickem's own colors and
 * legitimate caller styling survive. Pass `{ allowStyle: false }` to strip SGR too
 * (used for the active row, where the accent recolor must win).
 */
export function sanitizeDisplay(str: string, { allowStyle = true }: { allowStyle?: boolean } = {}): string {
  ...
  // Preserve SGR tokens; hard-scrub everything between them.
  const result = str
    .split(SGR_RE)
    .map((part, i) => (i % 2 === 1 ? part : scrubControls(part)))
    .join('')
  // Reset-terminate so unbalanced SGR can't bleed color into later rows/footer.
  // eslint-disable-next-line no-control-regex
