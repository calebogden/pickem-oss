...
import { displayWidth, sanitizeDisplay } from './width.js'
...
export function makeChrome({ colors, symbols }: ChromeDeps) {
  ...
  return {
    ...
    row({ label, meta = [], active = false, glyph = '', term = '' }: RowArgs): string {
      ...
      // injection (CWE-150) before it reaches the screen. Inactive rows keep caller
      // SGR color; the active row strips it so the accent recolor wins.
      const safe = sanitizeDisplay(label)
      const chevron = active ? `${accent(symbols.pointer)} ` : `${colors.conceal(symbols.pointer)} `
      const g = glyph ? `${glyph} ` : ''
      const safeMeta = meta.map((x) => sanitizeDisplay(x))
      const m = safeMeta.length ? dot + colors.dim(safeMeta.join(' · ')) : ''
      if (active) return `${chevron}${g}${colors.bold(accent(sanitizeDisplay(label, { allowStyle: false })))}${m}`
      if (!term) return `${chevron}${g}${safe}${m}`
      const i = safe.toLowerCase().indexOf(term.toLowerCase())
      const hl = i < 0 ? safe
        : safe.slice(0, i) + accent(safe.slice(i, i + term.length)) + safe.slice(i + term.length)
